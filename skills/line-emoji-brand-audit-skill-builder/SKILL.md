...
12. 日本語を含む `SKILL.md` / `agents/openai.yaml` を生成したら、直後に UTF-8 読みで確認し、Python 検証は `python -X utf8` を使う。
13. 作成後、重複するルール本文や長すぎるブランド説明を削り、参照パスと判断順だけで運用できる軽さにする。
14. skill の不足や blind spot が factory 側の再発問題なら、`../../workflows/continuous-improvement-workflow.md` に接続して記録する。
