...
- `固定IP不良`
  - 権利の所在、固定造形、可変域、禁止 drift が曖昧で、量産や審査に耐えない。
- `販売直前パッケージ不良`
  - export-ready asset、metadata、ファイル名、手作業提出メモが不足している。
- `運用記録不良`
...
- 固定IP設計: `workflows/fixed-ip-design-workflow.md`, `templates/fixed-ip-bible-template.md`
...
- 販売直前パッケージ: `workflows/sales-ready-package-workflow.md`, `templates/sales-package-manifest-template.md`
...
- 工程別 prompt 束へ転記すべき条件が特定されている。
- 完成後に sales package manifest へ落とすべき asset / metadata が特定されている。
- `release-checklist`, `quality-ledger`, `release-log` を初期化できる。

### Gate 0 で止めるべき状態
- set の目的がなく、単なる寄せ集めになっている。
- release 固有コンセプトがなく、ブランド説明の言い換えだけになっている。
- ブランド可変域が広すぎて、どこから drift か分からない。
- 共通固定条件が厚すぎて、ブランド固有の色が出せない。
...
- `固定IP不良` は `fixed-ip-bible` または `brand-system-prompt`
...
- 固定IPバイブルがある場合、final asset が固定造形と禁止 drift を守っている。
...
## Gate 3.5: export-ready package review
### 入力
- final asset
- `release-checklist`
...
### 確認すること
- export-ready asset と final candidate が混ざっていない。
- トークルームタブ画像とコンテンツ画像が揃っている。
- パッケージタイプに合うファイル名と個数になっている。
- metadata と画像内容が矛盾していない。
- 固定IPの権利と出自が説明できる。
- ユーザーが手作業で登録する項目が manifest から分かる。

### Gate 3.5 の原則
- `sales-package-manifest` が `Ready` でない限り、手作業提出へ渡さない。
- 審査提出操作はこの repo では実行せず、ユーザー手作業として release log へ戻す。
...
- 固定IPの drift が fixed IP bible と quality ledger で検出できるか。
- `Watch` が quality ledger に繋がっているか。
- release log に審査差し戻しと修正履歴が残るか。
- sales package manifest で手作業提出直前の asset と metadata が確認できるか。
