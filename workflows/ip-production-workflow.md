...
このファイルは、許諾済み固定IPをLINE絵文字として制作するときの標準手順を定義する。
...
3. 参照素材の出所、使用範囲、AI生成時の扱いを `reference-asset-register.md` に残す。
4. LINE絵文字としての使用場面を `構造 → ブランド → 商品` の順で確認する。
5. release spec に、固定IPらしさと日常使用の両方を落とす。
6. rough / final / revision の各段階で、キャラクター崩れと許諾外表現を確認する。
7. 権利者レビューが必要な場合は `ip-approval-log.md` を更新する。
8. 申請前に `workflows/submission-audit-workflow.md` へ接続する。

## 完了条件
- 許諾範囲と禁止事項が明文化されている。
- 使用した参照素材が登録されている。
- 承認が必要な箇所のログが残っている。
- LINE絵文字としての商品使用場面が成立している。
