...
- brand 側記録: `templates/qa/quality-ledger-template.md`, `templates/release/release-log-template.md`
- factory 側記録: `templates/improvement/factory-improvement-ledger-template.md`
...
8. skill の不足が原因なら、既存 skill 改修で足りるかを先に見る。足りなければ `skills/line-emoji-factory-auditor/SKILL.md` の `skill_audit` mode に接続する。
9. 次に効き目を見る地点を決める。
   - 次 release
   - 次ブランド探索
   - 次の rough review
...
14. `git status` を確認し、意図した差分だけを確認する。
15. 権限がある場合だけ commit / push する。権限がない場合は push 状態を `not pushed` として理由を書く。
...
## 出力原則
- すべての改善ループで `起点 / スコープ / 直す owner file / 検証 / 次観測点 / push 状態` を明示する。
