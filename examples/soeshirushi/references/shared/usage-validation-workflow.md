...
このファイルは、ブランド案や生成アイテムが
実際の会話文脈で本当に機能するかを検証する標準手順を定義する。

## 起動条件
- `release-spec` ができた。
- rough board や handoff ができた。
- 生成アイテムや final asset を監査したい。
- `買う理由` や `日常接続性` が設計上は見えるが、実例でまだ弱い。

## 使う正本
- ブランド核: `templates/brand/brand-setting-template.md`
- release 設計: `templates/release/release-spec-template.md`
- handoff 条件: `templates/release/production-handoff-template.md`
- 品質管理: `workflows/quality-control-workflow.md`
- 出力枠: `templates/qa/usage-validation-template.md`
- 継続改善: `workflows/continuous-improvement-workflow.md`

## 標準手順
1. 対象 release と対象アイテムを決める。
...
## 判定の目安
- `Pass`
  - 選択 item type を使う意味が明確で、代替表現より読ませ方に利点がある。
- `Revise`
  - 意味はあるが、近いアイテムや既存表現に負ける。
- `Cut`
