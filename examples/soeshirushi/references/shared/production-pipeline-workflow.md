...
このプロジェクトでは、制作方法を
`GPT-image2.0 で高品質なたたき台 / 全体図を大まかに作る → ClaudeDesign へ必要情報を渡し、1つ1つのアイテムとして完成させる`
で固定する。
...
- `ClaudeDesign` は受け渡された条件を元に、個別アイテムの完成工程を担う。
...
- 主要アイテムの方向、差分軸、全体トーン、並びの骨格を先に固定する。
...
### ClaudeDesign の責務
- ラフをそのまま清書することではなく、
  選択したLINE item typeとして成立する個別アイテムへ完成させること。
- ただし、ブランド核や set の差分軸を勝手に作り替えない。
- 1アイテムごとの意味、視認性、差分明確性、セット整合性を仕上げる。
...
- item type に応じて `rules/emoji-product-rules.md` または `rules/sticker-product-rules.md` と `rules/review-risk-rules.md` を通る。

## 差し戻しルール
- 問題が `構造不成立` なら、制作工程で直さず `workflows/transformation-workflow.md` へ戻す。
- 問題が brand 核の曖昧さや差分軸崩れなら、Stage 1 へ戻す。
- 問題が個別アイテムの仕上げ品質なら、Stage 2 で修正する。
