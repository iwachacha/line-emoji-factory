...
- `workflows/`
  - 手順の正本。
  - 相談処理、共通制作フロー、品質管理、変換処理、継続改善、探索手順、文書保守手順、ブランド運用手順を持つ。
- `templates/`
  - 出力骨格の正本。
  - ブランド設定、制作基盤、release 設計、handoff、QA、品質台帳、factory 改善台帳、探索 batch、repo 雛形、ブランド専用AI制作指示を持つ。
- `skills/`
  - 実務入口の正本。
...
19. 提出前と継続運用では `workflows/quality-control-workflow.md` を回し続ける。
20. 作業結果、監査結果、市場観測、skill friction は `workflows/continuous-improvement-workflow.md` に接続して、記録、昇格判断、軽量化判断まで閉じる。
21. ブランドの継続運用や独立制作が見えたら `workflows/brand-lifecycle-workflow.md` で残留 / 分離を決める。
22. 分離する場合は `templates/brand-repo-blueprint.md` と `templates/brand-repo-manifest-template.yaml` を使う。
23. 作業完了は、必要な owner file 更新、記録更新、remote push 完了まで含む。
